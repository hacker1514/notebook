...
about_notebook
echo ""
printf "${YELLOW}Detected OS: ${WHITE}Fedora${RESET}\n"
...
echo ""
printf "${CYAN}[_] Installing Notebook on Fedora...${RESET}\n"
echo ""
...
chmod +x "$BINARY_PATH"; cp "$BINARY_PATH" "${INSTALL_DIR}/notebook"
ln -sf "${INSTALL_DIR}/notebook" /usr/local/bin/notebook
if command -v restorecon >/dev/null 2>&1; then restorecon "${INSTALL_DIR}/notebook" /usr/local/bin/notebook 2>/dev/null || true; fi
